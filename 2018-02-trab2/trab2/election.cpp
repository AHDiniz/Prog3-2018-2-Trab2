...
string Election::electedCandidates() const
{
    ...
    // Getting each elected candidate:
    unsigned i = 0;
    for (Candidate *c : mostVoted)
    {
        if (i == vacancies)     // Breaking if all elected candidates were caught.
            break;
        if (c->getElected())    // Getting candidate if he was elected.
...
string Election::mostVotedCandidates() const
{
    ...
    // Getting the fist candidates in mostVoted until the number of vacancies:
    unsigned i = 1;
    for (Candidate *c : mostVoted)
    {
        ret << i++ << " - " << c->toString() << endl;
...
string Election::electedByMajority() const
{
    ...
    // Getting non elected candidates in the first positions of mostVoted until the number of vacancies:
    unsigned i = 1;
    for (Candidate *c : mostVoted)
    {
        if (i > vacancies)
...
string Election::electedByProportion() const
{
    ...
    // Getting elected candidates in mostVoted in positions beyond the number of vacancies:
    unsigned i = 1;
    for (Candidate *c : mostVoted)
    {
...
string Election::votesByCoalition() const
{
    ...
    vector<Coalition *> cList; // Vector of coalitions.

    // Adding each coalition to the vector:
    for (map<string, Coalition *>::iterator it = coalitions->begin(); it != coalitions->end(); it++)
    {
        cList.push_back(it->second);
    }

    sort(cList.begin(), cList.end(), Coalition::compare);   // Sorting the vector.

    // Getting each coalition on it:
    unsigned i = 0;
...
string Election::votesByParty() const
{
    ...
    vector<Party *> pList;  // Vector of parties.

    // Adding each party to the vector:
    for (map<string, Coalition *>::iterator it = coalitions->begin(); it != coalitions->end(); it++)
    {
    ...
    sort(pList.begin(), pList.end(), Party::compare);   // Sorting the vector.

    // Getting each party on it:
...
int Election::amountOfVotes() const
{
    ...
    // Counting the number of votes in each coalition:
    for (map<string, Coalition *>::iterator it = coalitions->begin(); it != coalitions->end(); it++)
    {
